...
* **[index.htmlをブラウザで起動してください](index.html)**
* **係数ボックス`(a, b)`に，`(1.9999, 1)`, `(1.99, 1)`, `(1.99, 0.99)`, `(1.98, 0.99)`を打ち込んで｢OK｣をクリックし，どんな曲線に急変するか楽しんでください．**
...
本教材は，従来の物理シミュレーション教育で頻繁に用いられる手法`[4]`の詳細な数学的解説を経ずに，シンプルな漸化式を動かすだけで，隣り合う数値同士の結びつきから生じるダイナミックな変化を視覚的かつ直感的に学べるように設計されています．
...
* **[index.html](index.html)**: 有限差分方程式（Finite Difference Equation）の係数をブラウザ上でインタラクティブに変更し，数値近似解（Numerical Approximate Solution）の挙動の変化（単振動から減衰振動，発散まで）を視覚的に探究できるHTMLアプリケーションです．
